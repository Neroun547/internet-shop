...
uploadProductForm.addEventListener("submit", async function (e) {
  ...
  const id = uploadProductForm.getAttribute("id");
  ...
  const api = await fetch("/admin/video-photo-gallery/edit-item/" + id, {
    method: "PATCH",
    body: formData
  });

  if(api.ok) {
    showModal("Публікацію відредаговано успішно", "/admin");
  } else {
